package io.funfunfine.api

import zhttp.http._
import zhttp.service._
import zio._
import zio.json._
import io.funfunfine.api.protocol._
import io.funfunfine.domain.{DomainError, ItemId}
import io.funfunfine.service.ItemService
import io.funfunfine.service.ItemService._
...
object HttpRoutes:
  ...
  val app: HttpApp[ItemService, Nothing] = Http.collectZIO {
    case Method.GET -> !! / "items" =>
      getAllItems().map(items =>
        Response.json(
          GetItems(
            items.map(item => GetItem(item.id.value, item.description))
          ).toJson
        )
      )
    ...
    case Method.GET -> !! / "items" / id =>
      getItemById(ItemId(id.toLong))
        .map {
          case Some(item) =>
            Response.json(GetItem(item.id.value, item.description).toJson)
    ...
    case req @ Method.POST -> !! / "items" =>
      (for
        body <- entity[CreateItem](req).absolve
    ...
    case req @ Method.PUT -> !! / "items" / id =>
      (for
        update <- entity[UpdateItem](req).absolve
                    .tapError(_ => ZIO.logInfo(s"Unparseable body "))
  ...
  private def entity[T: JsonDecoder](
      req: Request
  ): ZIO[Any, Throwable, Either[String, T]] =
