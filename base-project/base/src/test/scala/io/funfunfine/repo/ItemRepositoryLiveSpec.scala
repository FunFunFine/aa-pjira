...
import zio._
import io.funfunfine.repo.postgresql._
  ...
  override def spec =
    suite("item repository test with postgres test container")(
      test("save items returns their ids") {
        ...
        } yield (assert(id1)(equalTo(ItemId(1))) && assert(id2)(
          equalTo(ItemId(2))
        ) && assert(id3)(
          equalTo(ItemId(3))
        ))
      ...
      test("get item 2") {
        ...
        } yield (assert(item)(isSome) && assert(item.get.description)(
          equalTo("second item")
        ))
      },
      test("update item 3") {
