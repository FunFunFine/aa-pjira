...
object ItemServiceSpec extends ZIOSpecDefault:
  ...
  def spec = suite("item service test")(
    test("get item id accept long") {
      for
        found  <-
          assertZIO(getItemById(ItemId(123)))(isSome(equalTo(exampleItem)))
    ...
    suite("update item")(
      test("non existing item") {
        assertZIO(updateItem(ItemId(124), "bar").exit)(
          fails(
            equalTo(DomainError.BusinessError("Item with ID 124 not found"))
          )
        )
